...
import com.dvorobjov.UserManagementApplication;
...
public class WebApplicationSteps {
    ...
    @Then("the response status code should start with 2")
    public void statusCode2xx() {
        assertThat(Integer.toString(World.getInstance().getCurrentHar().getResponse().getStatus()).substring(0, 1), equalTo("2"));
    }

    @Then("the response page is exists")
    public void responsePageExists() {
        assertNotNull(((UserManagementApplication)getWebApplication()).getLastResponsePageSource());
    }
